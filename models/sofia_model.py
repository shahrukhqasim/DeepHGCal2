...
def sofiamodel(Inputs, nclasses, nregressions, dropoutRate=0.05, momentum=0.6):
    ...
    x = Convolution3D(32, kernel_size=(1, 1, 1), strides=(1, 1, 1), activation='relu',
                      kernel_initializer='lecun_uniform', kernel_regularizer=l2(l2_lambda))(x)
    x = Dropout(dropoutRate)(x)
    x = Convolution3D(12, kernel_size=(1, 1, 1), strides=(1, 1, 1), activation='relu',
    ...
    smallshower = Convolution3D(32, (3, 3, 5), strides=(2, 2, 3), border_mode='same', name='muon0',
                                kernel_regularizer=l2(l2_lambda), )(smallshower)
    smallshower = LeakyReLU()(smallshower)
    smallshower = BatchNormalization()(smallshower)
    smallshower = Dropout(dropoutRate)(smallshower)
    smallshower = Convolution3D(16, (1, 1, 5), strides=(1, 1, 3), border_mode='same', name='muon1',
                                kernel_regularizer=l2(l2_lambda), )(smallshower)
    smallshower = LeakyReLU()(smallshower)
    smallshower = BatchNormalization()(smallshower)
    smallshower = Dropout(dropoutRate)(smallshower)
    smallshower = Convolution3D(4, (1, 1, 5), strides=(1, 1, 3), border_mode='same', name='muon2',
                                kernel_regularizer=l2(l2_lambda), )(smallshower)
    smallshower = LeakyReLU()(smallshower)
    smallshower = BatchNormalization()(smallshower)
    smallshower = Dropout(dropoutRate)(smallshower)
    ...
    merged = Concatenate()([globals, x, totalrecenergy,
                            flattenedsmall])  # add the inputs again in case some don't like the multiplications
